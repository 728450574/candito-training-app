<template>
  <main class="pb-8 max-w-lg mx-auto px-4">
    ...
    <header class="flex items-center h-11 px-1 mb-2">
      ...
      <h1 class="flex-1 text-center font-semibold truncate" style="font-family: var(--font-sans); font-size: var(--text-md); color: var(--color-primary);">暂停周期</h1>
      <div class="w-8"></div>
    </header>
    ...
    <template v-else>
      <section class="px-1 mt-2 mb-5">
        <div class="rounded-[var(--radius-lg)] px-4 py-3" style="background: var(--color-surface-muted);">
          <p class="truncate" style="font-family: var(--font-sans); font-size: var(--text-sm); color: var(--color-primary-light);">当前进度</p>
          <p class="font-semibold truncate mt-0.5" style="font-family: var(--font-sans); font-size: var(--text-base); color: var(--color-primary);">{{ progressLabel }}</p>
        </div>
      </section>

      <section class="px-1 mb-5">
        <p class="px-3 mb-2 font-medium" style="font-family: var(--font-sans); font-size: var(--text-sm); color: var(--color-primary-light);">暂停原因</p>
        <div class="rounded-[var(--radius-lg)] overflow-hidden" style="background: var(--color-surface); box-shadow: var(--shadow-card);">
          <button
            ...
            :class="{ 'border-b': idx < reasonOptions.length - 1 }"
            :style="{
              borderColor: 'var(--color-border-light)',
              background: selectedReason === opt.value ? 'var(--state-info-bg)' : ''
            }"
            @click="selectedReason = opt.value"
          >
            <component :is="opt.icon" class="w-5 h-5 shrink-0" style="color: var(--color-primary-light);" />
            <span class="flex-1 truncate" style="font-family: var(--font-sans); font-size: var(--text-base); color: var(--color-primary);">{{ opt.label }}</span>
            <Check v-if="selectedReason === opt.value" class="w-5 h-5 shrink-0" style="color: var(--color-training-main);" />
          </button>
        </div>
      </section>

      <section v-if="selectedReason === 'other'" class="px-1 mb-5">
        <div class="rounded-[var(--radius-lg)] px-4 py-3" style="background: var(--color-surface); box-shadow: var(--shadow-card);">
          <input
            v-model="customReason"
            type="text"
            placeholder="请输入暂停原因（选填）"
            class="w-full outline-none bg-transparent"
            style="font-family: var(--font-sans); font-size: var(--text-base); color: var(--color-primary);"
          >
        </div>
      </section>

      <section class="px-1 mb-5">
        <p class="px-3 mb-2 font-medium" style="font-family: var(--font-sans); font-size: var(--text-sm); color: var(--color-primary-light);">预计暂停时长</p>
        ...
        <div class="flex gap-2 mb-3 px-1">
          <button
            ...
            :style="durationBtnStyle(opt.value)"
        ...
        <div v-if="selectedDuration === 'custom'" class="px-1">
          <div class="flex items-center gap-3 rounded-[var(--radius-lg)] p-3" style="background: var(--color-surface); box-shadow: var(--shadow-card);">
            <div class="flex-1 text-center">
              <p style="font-family: var(--font-sans); font-size: var(--text-xs); color: var(--color-primary-light);">开始</p>
              <p class="font-semibold mt-0.5" style="font-family: var(--font-sans); font-size: var(--text-base); color: var(--color-primary);">{{ customStartDate }}</p>
            </div>
            ...
            <div class="flex-1 text-center">
              ...
              <p class="font-semibold mt-0.5" style="font-family: var(--font-sans); font-size: var(--text-base); color: var(--color-primary);">{{ customEndDate }}</p>
            </div>
          </div>
        </div>

        <div class="px-1 mt-3 text-center">
          <p style="font-family: var(--font-mono); font-size: var(--text-lg); font-weight: var(--font-weight-semibold); color: var(--color-training-main);">共 {{ totalDaysNum }} 天</p>
        </div>
      </section>

      <section class="px-1 mb-8">
        <div class="rounded-[var(--radius-lg)] p-4" style="background: var(--color-surface); box-shadow: var(--shadow-card);">
          <p class="font-semibold mb-3" style="font-family: var(--font-sans); font-size: var(--text-base); color: var(--color-primary);">受影响训练: {{ affectedDays.length }}个训练日</p>
          <div class="space-y-2">
            <div v-for="day in affectedDays" :key="day.key" class="flex items-center gap-3 py-1.5">
              <span class="inline-flex items-center shrink-0 rounded-[var(--radius-sm)] px-2 py-0.5 font-medium whitespace-nowrap" style="font-family: var(--font-mono); font-size: var(--text-xs); color: var(--color-training-main); background: var(--state-info-bg);">{{ day.label }}</span>
              <span class="flex-1 truncate" style="font-family: var(--font-sans); font-size: var(--text-sm); color: var(--color-primary);">{{ day.date }}</span>
            </div>
          </div>
        </div>
      </section>
...
function durationBtnStyle(value: string) {
  if (value === selectedDuration.value) {
    return {
