...
Component({
  ...
  methods: {
    recompute() {
      const ex = this.data.exercise as ExerciseRecord | undefined
      if (!ex || !ex.name) return

      // 类型标签与徽章样式
      let typeLabel = '补充项'
      let badgeStyle =
        'font-family: var(--font-sans); font-size: var(--text-xs); font-weight: var(--font-weight-semibold); color: var(--color-training-assist); background: rgba(94, 92, 230, 0.08);'
      if (ex.type === 'main') {
        typeLabel = '主项'
        badgeStyle =
