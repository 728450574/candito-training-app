...
Page({
    ...
    recompute() {
        ...
        const completedWeekCount = activeCycle.weeks.filter((w) => w.days.filter((d) => d.type !== 'rest').every((d) => d.status === 'completed')).length;
        ...
        const formattedStartDate = (0, dateService_1.formatDateFull)(activeCycle.startDate);
        const oneRMCards = [
    ...
    buildWeekTimeline(cycle) {
        ...
        return cycle.weeks.map((week) => {
            ...
            const completedDays = trainingDays.filter((d) => d.status === 'completed').length;
            let state = 'pending';
            if (completedDays === totalDays && totalDays > 0) {
                state = 'completed';
            }
            else if (week.days.some((d) => d.scheduledDate === today) && completedDays < totalDays) {
                state = 'in-progress';
            }
            else if (completedDays > 0 && completedDays < totalDays) {
                state = 'in-progress';
            }
            const cardStyle = state === 'in-progress'
                ? 'background: var(--state-info-bg); box-shadow: var(--shadow-card);'
                : 'background: var(--color-surface); box-shadow: var(--shadow-card);';
            const titleStyle = state === 'pending'
    ...
    buildPauseHistory(pauseHistory) {
        const reasonMap = {
            holiday: '假期',
            travel: '出差',
            injury: '受伤',
    ...
    buildHistoricalCycles() {
        const completed = cycleStore_1.cycleStore.getCompletedCycles();
        return completed.map((cycle) => {
            const statusLabel = cycle.status === 'completed' ? '已完成' : '已终止';
            const statusBadgeStyle = cycle.status === 'completed'
    ...
    cycleSubtitle(cycle) {
        if (cycle.status === 'completed') {
            const weeksDone = cycle.weeks.filter((w) => w.days.filter((d) => d.type !== 'rest').every((d) => d.status === 'completed')).length;
    ...
    restartCurrent() {
        ...
        const data = (0, planGenerator_1.createCycle)({
            oneRM: cycle.oneRM,
            unit: cycle.unit,
            weightRounding: cycle.weightRounding,
            startDate: (0, dateService_1.getToday)(),
            assistanceConfig: cycle.assistanceConfig,
        });
        cycleStore_1.cycleStore.updateCycle(cycle.id, {
            status: 'completed',
            completedAt: (0, dateService_1.getToday)(),
        });
        cycleStore_1.cycleStore.addCycle(data);
            ...
            success: (res) => {
                if (res.confirm) {
                    cycleStore_1.cycleStore.updateCycle(activeCycle.id, {
                        status: 'terminated',
                        terminatedAt: (0, dateService_1.getToday)(),
                        terminateReason: '因伤终止',
                    });
                    cycleStore_1.cycleStore.setActiveCycle('');
                }
            },
        });
    },
