...
Page({
    ...
    onLoad(options) {
        ...
        const cycle = cycleId ? cycleStore_1.cycleStore.getCycleById(cycleId) : cycleStore_1.cycleStore.getActiveCycle();
        if (!cycle) {
        ...
        this._defaultRestSeconds = settingsStore_1.settingsStore.getSettings().defaultRestSeconds;
        // 设置 pageTitle
        const typeLabelMap = { lower: '下肢训练', upper: '上肢训练' };
        ...
        // 构建可变动作列表
        this._exercises = dayData.exercises.map((ex) => ({
            id: ex.id,
            name: ex.name,
            type: ex.type,
            sets: ex.sets.map((s) => ({
    ...
    async finishWorkout() {
        ...
        const todayStr = (0, dateService_1.getToday)();
        const exerciseRecords = this._exercises.map((ex) => ({
            exerciseId: ex.id,
            name: ex.name,
            type: ex.type,
            sets: ex.sets.map((s) => ({
        ...
        this.clearDraft();
        await recordStore_1.recordStore.addRecord(record);
        // 更新周期天状态
            ...
            return {
                ...
                days: week.days.map((day) => {
                    if (day.dayNumber !== this._dayNum)
                        return day;
                    return { ...day, status: 'completed', completedDate: todayStr };
                }),
            };
        });
        ...
        if (this._weekNum === 5) {
            ...
            const week5Done = week5 && week5.days.filter((d) => d.type !== 'rest').every((d) => d.status === 'completed');
            if (week5Done) {
                cycleStore_1.cycleStore.updateCycle(cycle.id, { weeks: updatedWeeks, status: 'week6_pending' });
                wx.redirectTo({ url: `/pages/week6/index?cycleId=${cycle.id}` });
                return;
            }
        }
        cycleStore_1.cycleStore.updateCycle(cycle.id, { weeks: updatedWeeks });
        wx.redirectTo({
    ...
    recomputeRepDisplay() {
        ...
        const repOptionsDisplay = (options.length >= 3 ? options : [center]).map((value) => ({
            value,
