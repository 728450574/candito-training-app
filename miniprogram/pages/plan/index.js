...
Page({
    ...
    recompute() {
        ...
        let displayCycle = null;
        if (cycleId) {
            displayCycle = cycleStore_1.cycleStore.getCycleById(cycleId) || null;
        }
        else {
            displayCycle = cycleStore_1.cycleStore.getActiveCycle();
        }
        ...
        let dayCards = [];
        if (selectedWeekData) {
            const trainingDays = selectedWeekData.days.filter((d) => d.type !== 'rest');
            dayCards = trainingDays.map((day) => this.buildDayCard(day));
        }
        this.setData({
    ...
    dayStatus(day) {
        if (day.status === 'completed')
            return 'completed';
        if (day.status === 'pending' && day.scheduledDate === (0, dateService_1.getToday)())
            return 'in-progress';
        if (day.scheduledDate < (0, dateService_1.getToday)() && day.status === 'pending')
            return 'pending';
        return 'pending';
    },
    buildDayCard(day) {
        ...
        const weekdayLabel = (0, dateService_1.getWeekday)(day.scheduledDate);
