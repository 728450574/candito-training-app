...
Page({
  data: {
    ...
    changeIndicator: '—',
    lastRecordDate: '暂无记录',
    displayHistory: [] as HistoryDisplayItem[],
  },

  _unsub: null as null | (() => void),
  _chartPoints: [] as ChartPoint[],
  _chartYMax: 83,
  _chartYMin: 80,
  _chartYMidHigh: 82,
  _chartYMidLow: 81,
  ...
  onLoad() {
    this._unsub = bodyMetricStore.subscribe(() => this.recompute())
    this.setData({ recordDateDisplay: formatDateFull(this.data.recordDate) })
    this.recompute()
  },
  ...
  recompute() {
    const metrics = bodyMetricStore.getMetrics()
    ...
    if (sortedDesc.length > 0) {
      ...
      lastRecordDate = formatDateFull(sortedDesc[0].date)
    }
    ...
    if (weightDiff > 0) changeColor = 'var(--state-warning)'
    else if (weightDiff < 0) changeColor = 'var(--state-success)'
    ...
    const chartPoints: ChartPoint[] = chartItems.map((m) => ({
    ...
    const displayHistory: HistoryDisplayItem[] = items.map((m, i) => {
  ...
  renderChart(ctx: CanvasRenderingContext2D, width: number, height: number) {
  ...
  handleDateChange(e: WechatMiniprogram.PickerChange) {
    ...
    this.setData({
      recordDate: date,
      recordDateDisplay: formatDateFull(date),
    })
  },
