...
interface AssistanceItem {
  key: 'horizontalPull' | 'shoulder' | 'verticalPull'
  cn: string
  en: string
  current: string
}
...
const WEEK_PCTS: Record<LiftKey, number[]> = {
  squat: SQUAT_PCTS,
  bench: BENCH_PCTS,
  deadlift: DEADLIFT_PCTS,
}

type AssistanceCategory = 'horizontalPull' | 'shoulder' | 'verticalPull'

const ASSISTANCE_SELECTIONS: Record<AssistanceCategory, string[]> = {
...
Page({
  data: {
    ...
    showAssistance: true,
    lifts: [] as LiftCard[],
    assistanceItems: [] as AssistanceItem[],
  ...
  _rounding: 2.5,
  _oneRM: { squat: 100, bench: 85, deadlift: 120 } as Record<LiftKey, number>,
  _currentSelectionIndex: {
    horizontalPull: 0,
    shoulder: 0,
    verticalPull: 0,
  } as Record<AssistanceCategory, number>,
  _assistanceCurrent: {
    horizontalPull: '哑铃划船',
    shoulder: '坐姿哑铃推举',
    verticalPull: '负重引体向上',
  ...
  onLoad() {
    ...
    // 同步 selection index
    ;(Object.keys(ASSISTANCE_SELECTIONS) as Array<keyof typeof ASSISTANCE_SELECTIONS>).forEach((key) => {
  ...
  recompute() {
    ...
    const previewRows: PreviewRow[] = ([
      ...
      { key: 'bench', cn: '卧推' },
      { key: 'deadlift', cn: '硬拉' },
    ] as Array<{ key: LiftKey; cn: string }>).map((l) => {
      const pcts = WEEK_PCTS[l.key]
  ...
  handleWeightInput(e: WechatMiniprogram.Input) {
    const key = e.currentTarget.dataset.key as LiftKey
  ...
  switchAssistance(e: WechatMiniprogram.TouchEvent) {
    const key = e.currentTarget.dataset.key as 'horizontalPull' | 'shoulder' | 'verticalPull'
  ...
  handleSave() {
    ...
    const newWeeks = buildWeeks(
      { squat: this._oneRM.squat, bench: this._oneRM.bench, deadlift: this._oneRM.deadlift },
      this._rounding,
      newConfig,
