...
class RecordStore {
    ...
    async loadRecords(cycleId) {
        try {
            const records = await storageManager_1.storageManager.getActiveAdapter().get(this.storageKey(cycleId));
    ...
    async getRecordsForCycle(cycleId) {
        if (!this.recordsByCycle[cycleId]) {
    ...
    async getRecordsForWeek(cycleId, weekNumber) {
    ...
    async getRecordForDay(cycleId, weekNumber, dayNumber) {
        const records = await this.getRecordsForCycle(cycleId);
...
exports.recordStore = new RecordStore();
