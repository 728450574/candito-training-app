...
### 1.3 各周详细训练日（Excel 原始计划 vs 代码实现）

> 以下数据基于示例 1RM：深蹲 100kg / 卧推 90kg / 硬拉 120kg

#### 第1周 — 肌肉调理（5天）

| Day | 偏移 | 类型 | Excel 原始计划 | 代码实现（planGenerator.ts） |
|-----|------|------|---------------|-----------------------------|
| D1 | +0 | 下肢 | 深蹲 80kg×6×4组, 硬拉 75kg×6×2组, 可选 | 深蹲 80kg×6×2组 ✅, 硬拉 75kg×6×2组 ✅, 无可选 |
| D2 | +1 | 上肢 | 卧推(45×10→60×10→67.5×8→70×6)金字塔, 辅助项, 可选 | 卧推金字塔同 Excel ✅, 辅助项 ✅, 无可选 |
| D3 | +3 | 上肢 | 卧推金字塔（同 D2）, 辅助项, 可选 | 卧推金字塔同 D2 ✅, 辅助项 ✅, 无可选 |
| D4 | +4 | 下肢 | 深蹲 70kg×8×2组, 硬拉 65kg×8×2组, 可选 | 深蹲 75kg×8×2组 ✅, 硬拉 70kg×8×2组 ✅, 无可选 |
| D5 | +5 | 上肢 | 卧推 72.5kg×MR, 辅助项, 可选 | 卧推 67.5kg×MR ✅, 辅助项 ✅, 无可选 |

**差异说明**:
- D4 深蹲：Excel 是 80%×8（80kg），代码是 75%×8（75kg），取的是 W=4 区间下限
- D4 硬拉：Excel 是 65kg（~54%），代码是 70%×8（84kg），代码更接近 Excel 的 W1 硬拉值
- D1 深蹲：Excel 写 4组，代码是 2组（系统组数偏少）
- D3/D5 早期缺失卧推主项（2026-07-10 已补全）

#### 第2周 — 肌肉调理/增肌（5天）

| Day | 偏移 | 类型 | Excel 原始计划 | 代码实现 |
|-----|------|------|---------------|---------|
| D1 | +0 | 下肢 | 深蹲 80kg×MR10, 深蹲加量组, 硬拉变式×8×3, 可选 | 深蹲 80kg×MR10 ✅, 加量组动态生成 ✅, 硬拉变式 84kg×8×3 ✅, 可选 ✅ |
| D2 | +1 | 上肢 | 卧推(65×10→70×8→75×6-8)×3, 辅助项, 可选 | 卧推 65.3kg(72.5%)×8×3 ✅, 辅助项 ✅, 无可选 |
| D3 | +3 | 下肢 | 深蹲 82.5kg×MR10, 深蹲减量组, 硬拉变式×8×3, 可选 | 深蹲 82.5kg×MR10 ✅, 减量组动态生成 ✅, 硬拉变式 84kg×8×3 ✅ |
| D4 | +4 | 上肢 | 卧推(65×10→70×8→75×6-8)×3, 辅助项, 可选 | 卧推 63kg(70%)×8×3 ✅, 辅助项 ✅ |
| D5 | +6 | 上肢 | 卧推 70kg×MR, 辅助项, 可选 | 卧推 67.5kg(75%)×MR ✅（2026-07-10 补全） |

**MR10 动态调整**:
- D1 深蹲 MR10 ≥ 8次 → 加量组 5组×3次 +2.5kg；< 8次 → 减轻2.5%后加量组
- D3 深蹲 MR10: 10次→10组, 8-9次→8组, 7次→5组, <7次→跳过并提示降低1RM

#### 第3周 — 线性最大超负荷（4天）

| Day | 偏移 | 类型 | Excel 原始计划 | 代码实现 |
|-----|------|------|---------------|---------|
| D1 | +0 | 下肢 | 深蹲 87.5kg×4-6×3组, 硬拉 82.5kg×3-6×2组 | 深蹲(87.5→90)×3×2组 ✅, 硬拉 82.5kg×3×2组 ✅ |
| D2 | +2 | 上肢 | 卧推 77.5kg×4-6×3组, 辅助项×6×3组 | 卧推(77.5→80)×4×2组 ✅, 辅助项 ✅ |
| D3 | +4 | 下肢 | 深蹲 90kg×4-6×1组, 硬拉变式×8×1组 | 深蹲 85kg×3×2组, 硬拉 80kg×3×2组 |
| D4 | +5 | 上肢 | 卧推 80kg×4-6×3组, 辅助项×6×3组 | 卧推 75kg×4×3组 ✅, 辅助项 ✅ |

**差异**:
- D3 代码是 2个主项（深蹲+硬拉变式），各2组，与 Excel 的 1组不同
- 代码每组都是固定次数（3或4），Excel 是区间（4-6）

#### 第4周 — 适应大重量（4天）

| Day | 偏移 | 类型 | Excel 原始计划 | 代码实现 |
|-----|------|------|---------------|---------|
| D1 | +0 | 下肢 | 深蹲(87.5→90→92.5)×3×3组, 硬拉变式×6×2组 | 深蹲(85→90→92.5)×3×3组 ✅, 硬拉 87.5→90×3×2组 |
| D2 | +1 | 上肢 | 卧推(75→75→80)×3×3组, 辅助项, 可选 | 卧推(80→85→87.5)×3×3组, 辅助项 ✅ |
| D3 | +3 | 下肢 | 深蹲(92.5×3→95×1-2), 硬拉(87.5×3→90×1-2) | 深蹲 87.5kg×3×2组 ✅, 硬拉 85kg×3×2组 ✅ |
| D4 | +4 | 上肢 | 卧推(80×3→80×2-4→85×1-2), 辅助项 | 卧推 82.5kg×3×2组 ✅, 辅助项 ✅ |

**差异**: D2 卧推代码用的是 80%/85%/87.5% 比 Excel 略高

#### 第5周 — 高强度力量训练（3天）

| Day | 偏移 | 类型 | Excel 原始计划 | 代码实现 |
|-----|------|------|---------------|---------|
| D1 | +0 | 下肢 | 深蹲 97.5kg×1-4, 硬拉(65×4→67.5×4→70×2) | 深蹲 97.5kg×AMRAP(1-4) ✅ |
| D2 | +2 | 上肢 | 卧推 87.5kg×1-4, 辅助项 | 卧推 78.8kg×AMRAP(1-4), 辅助项 ✅ |
| D3 | +4 | 下肢 | 硬拉 92.5kg×1-4 | 硬拉 111kg×AMRAP(1-4) ✅ |

**差异**: D2 代码使用 87.5%×90kg≈78.8kg，Excel 是 87.5kg

### 1.4 代码 vs Excel 已解决的主要差异

| 问题 | 状态 | 修复说明 |
|------|------|---------|
| 第1周训练日数量 | ✅ 已解决 | 现为5天（原系统只有4天） |
| W1D3 无上肢主项 | ✅ 已解决 | 已补卧推金字塔 |
| W1D5 缺失 | ✅ 已解决 | 已补卧推 MR 组 |
| W2D5 无 MR 组 | ✅ 已解决 | 已补卧推 MR 组 |
| 辅助项不可切换 | ✅ 已解决 | reactive + type="button" |
| 容量未记录 | ✅ 已解决 | WorkoutRecord 存储每组 actualWeight×actualReps |
| 训练日组数偏少 | ⚠️ 与 Excel 有差异 | 代码统一 `mainSets(2, ...)`，与 Excel 不完全一致 |
